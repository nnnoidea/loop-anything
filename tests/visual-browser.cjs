...
engine.close();print(json.dumps(runs))`;
 ...
 try{
  ...
  for(const [index,id] of ids.entries()){
   ...
   await page.locator('[data-task-focus="initial"]').click();assert((await page.locator('#task-detail').innerText()).includes(index===0?'质量':'铁路'));
   ...
   await page.locator(`[data-task-focus="${task.id}"]`).click();await page.locator('[data-edit-task]').click();await page.locator('#task-parameters input[type="number"]').fill('5');await page.locator('#task-edit-reason').fill('调整后续参数');await apply();r=await getRun(id);assert.equal(r.tasks[task.id].spec.parameters.limit,5);assert.deepEqual(r.records,original.records);assert.deepEqual(r.executions,original.executions);
   ...
   await page.waitForTimeout(1600);await page.locator(`[data-task-focus="${task.id}"]`).click();assert((await page.locator('#task-detail').innerText()).includes('Checked'));await page.locator('[data-source-execution]').click();assert((await page.locator('#task-detail').innerText()).includes(index===0?'质量':'铁路'));
  ...
  await page.locator('[data-copy-loop-definition]').first().click();
  await page.locator('[data-edit-edge="next"]').click();
  await page.locator('#editor-save').click();await page.locator('#editor-save-status').filter({hasText:'已保存'}).waitFor();
  ...
  await page.locator('#add-node').click();await page.locator('#property-title').filter({hasText:'node_2'}).waitFor();await page.locator('[data-add-author-port="inputs"]').click();await page.locator('#author-inputs > details > summary').click();await page.locator('#author-inputs [data-port-name]').fill('source');await page.locator('#author-node-form button[type="submit"]').click();
  ...
  await page.locator('#editor-save').click();await page.locator('#editor-save-status').filter({hasText:'已保存'}).waitFor();drafts=await (await page.request.get(base+'/api/drafts')).json();d=drafts.find(d=>d.loop_definition.nodes.node_2);assert.deepEqual(d.loop_definition.plans.default.steps.node_2.inputs.source,{from:'node_1',port:'result'});
