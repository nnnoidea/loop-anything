...
function loopNodePositions(bp,{rowGap=210,maxColumns=3}={}){
  ...
  const positions=new Map(),columns=Math.max(1,Math.min(maxColumns,ids.length));
  ...
  ordered.forEach((id,i)=>{
    ...
    let p;do{const row=Math.floor(i/columns),col=row%2?columns-1-i%columns:i%columns;p={x:60+col*250,y:85+row*rowGap};i++;}while(occupied.some(o=>Math.abs(o.x-p.x)<210&&Math.abs(o.y-p.y)<rowGap-30));
...
function loopEdgePath(e,a,b,i=0){
  let d,lx,ly;
    ...
    else {const down=b.y>a.y,sy=down?a.y+(a.height||90):a.y,ty=down?b.y:b.y+(b.height||90),mid=(sy+ty)/2+(e.kind==='planning'?12:0);d=`M ${a.x+100} ${sy} C ${a.x+100} ${mid},${b.x+100} ${mid},${b.x+100} ${ty}`;}
  return {d,lx,ly};
}
...
function waitSummary(config){
  if(!config)return '';
  const value=v=>v&&typeof v==='object'?(Object.hasOwn(v,'$')?'引用 '+(Array.isArray(v.$)?v.$.join('.'):v.$):pretty(v)):String(v??'');
  if(config.kind==='timer')return config.wait?.until!==undefined?'◷ 等到 '+value(config.wait.until):'◷ 等待 '+value(config.wait?.seconds??'?')+' 秒';
  if(config.kind==='event')return '◇ 等待事件 '+(config.event||'待设置')+(config.wait?.key?' · '+value(config.wait.key):' · 按任务关联标识')+(config.wait?.timeout!==undefined?' · '+value(config.wait.timeout)+' 秒超时':'');
  if(config.kind==='approval')return '等待用户确认';
  return '';
}
function nodeNotices(item,id,config){
  const authored=(lifecycleOf(config)?.transitions||[]).flatMap(rule=>(rule.notify||[]).map(notice=>({stage:lifecycleName(rule.from)+' → '+lifecycleName(rule.to),event:lifecycleEventName(config,rule.event),when:rule.when,...notice})));
  const hooks=(item.settings?.hooks||[]).filter(h=>h.enabled!==false&&h.action==='notify'&&(h.target.node===id||item.tasks?.[h.target.tasks]?.spec.node===id)).map(h=>({stage:({before:'派发前',after:'完成后',transition:'状态转移时'})[h.phase],event:[h.event&& (lifecycleEvents[h.event]||h.event),h.to&&('进入 '+lifecycleName(h.to)),h.target.tasks&&('任务 '+h.target.tasks)].filter(Boolean).join(' · '),message:h.message||'任务通知',route:h.route||'default'}));
  return authored.concat(hooks);
}
function nodeSignalsHTML(item,id,config){
  const wait=waitSummary(config),notices=nodeNotices(item,id,config),conditional=Object.entries(item.loop_definition.plans||{}).flatMap(([plan,p])=>Object.entries(p.steps).filter(([,s])=>s.node===id&&s.when).map(([step])=>plan+' / '+step));
  return `<div class="node-signals">${conditional.length?`<span class="node-conditional" title="${esc(conditional.join('；'))}">◇ 含条件安排 · ${conditional.length} 个模板步骤</span>`:''}${wait?`<span class="node-wait" title="${esc(wait)}">${esc(wait)}</span>`:''}${notices.length?`<span class="node-notifies" title="${esc(notices.map(n=>n.stage+' · '+n.event).join('；'))}">◇ ${esc([...new Set(notices.map(n=>n.event||n.stage))].join(' / '))}通知 · ${notices.length}</span>`:''}</div>`;
}
function noticeScheduleHTML(item,bindings={}){
  const rows=Object.entries(item.loop_definition.nodes).flatMap(([id,n])=>nodeNotices(item,id,chosenImplementation(item,id,bindings)).map(notice=>({...notice,node:n.label||id})));
  return `<details class="notice-schedule" open><summary>通知发生在什么时候 · ${rows.length} 项安排</summary><p class="small muted">按当前选用的实现展示；切换实现会采用对应规则。脚本或 Agent 临时发送的通知见运行记录。</p>${rows.length?`<div class="matrix-scroll"><table><thead><tr><th>节点 / 阶段</th><th>通知 / 询问</th><th>出口</th></tr></thead><tbody>${rows.map(n=>`<tr><td><strong>${esc(n.node)}</strong><small>${esc(n.stage)} · ${esc(n.event)}${n.when?' · 满足条件时':''}</small>${n.when?`<details><summary>条件</summary><pre>${esc(pretty(n.when))}</pre></details>`:''}</td><td>${esc(n.message)}${n.reply?'<small>需要回复；是否等待由等待任务决定</small>':''}</td><td>${esc(n.route==='workspace'?'平台通知区':!n.route||n.route==='default'?'本次 Run 的通知出口':n.route)}</td></tr>`).join('')}</tbody></table></div>`:'<p class="small muted">当前实现没有声明阶段通知。</p>'}<p class="small">平台还会在 Run 完成、Agent 连续失败三次并暂停时通知。实际发送目标由本次 Run 选择。</p></details>`;
}
function executionOverviewHTML(item,bindings={},editable=false){
  return `<div class="execution-overview"><p class="small muted">${item.tasks?'以下展示本 Run 默认选用的实现；实际任务的状态与执行快照在上方运行过程中查看。':'先看平台会做什么，再按需展开完整状态图。'}箭头表示「当前状态 + 报告事件 / 条件 → 下一状态」；节点间的依赖另看任务关系。</p><div class="execution-overview-grid">${Object.entries(item.loop_definition.nodes).map(([id,n])=>{const config=chosenImplementation(item,id,bindings),selected=Object.hasOwn(bindings,id)?bindings[id]:defaultImplementation(item.implementations[id]);return `<section class="execution-overview-node"><h3>${esc(n.label||id)}</h3><p class="small muted">${esc(mapImplementationLabel(item,id,bindings))}</p>${nodeSignalsHTML(item,id,config)}${config?executionResponsibilities(config)+executionRulesSummary(config)+`<details><summary>完整状态图与矩阵</summary>${lifecycleMatrix(config)}</details>`:'<p>尚未选用实现。</p>'}${editable?`<button type="button" data-editor-node="${esc(id)}" ${selected?`data-editor-implementation="${esc(selected)}"`:''}>编辑执行方式与处理规则 ↗</button>`:''}</section>`;}).join('')}</div></div>`;
}
...
function loopGraphHTML(item,{bindings={},focus='',action='detail',fallbackNode}={}){
  ...
  const positions=loopNodePositions(bp,{rowGap:250});for(const p of positions.values())p.height=164;const ordered=[...positions.keys()],width=Math.max(400,...[...positions.values()].map(p=>p.x+260)),height=Math.max(320,...[...positions.values()].map(p=>p.y+230));
  ...
  const cards=ordered.map(id=>{const p=positions.get(id),n=bp.nodes[id],tasks=Object.values(item.tasks || {}).filter(t=>t.spec.node===id),attempts=(item.executions || []).filter(e=>e.node===id),kind=chosenImplementation(item,id,bindings)?.kind,repeat=edges.some(e=>e.from===id&&e.repeats);
    return `<button type="button" class="map-node ${['timer','event','approval'].includes(kind)?'is-wait':''}" data-map-node="${esc(id)}" ${action==='run'?`data-node="${esc(id)}"`:''} style="left:${p.x}px;top:${p.y}px" aria-pressed="false"><small>${id===fallback?'异常兜底':id===bp.entry?'入口':repeat?'↻ 可再次安排工作':kindNames[kind] || '待接入'}</small><strong>${esc(label(id))}</strong><span class="map-current-implementation" title="${esc(mapImplementationLabel(item,id,bindings))}">${esc(mapImplementationLabel(item,id,bindings))}</span>${nodeSignalsHTML(item,id,chosenImplementation(item,id,bindings))}${action==='run'?`<span>${tasks.length?`${tasks.length} 项任务 · ${attempts.length} 次执行`:'未安排 Task'}</span>`:''}</button>`;
  }).join('');
  ...
  return `<section class="loop-map" data-map-focus="${esc(focus)}"><div class="loop-map-heading"><div><strong>Loop 的任务关系与状态机</strong><p class="small muted">关系图表达任务依赖和可安排范围；执行状态机展示真正的状态转移。</p><div class="graph-layer-switch"><button type="button" data-graph-layer="relations" aria-pressed="true">任务关系</button><button type="button" data-graph-layer="states" aria-pressed="false">执行状态机</button></div></div><label class="map-expand"><input type="checkbox" data-map-expand>全部安排连线</label></div><div class="map-legend"><span class="dependency">实线 · 数据 / 先后依赖</span><span class="planning">虚线 · 可安排新任务（非自动跳转）</span><span class="repeat">↻ 再次安排该节点的新任务</span><span class="recovery">红色虚线 · 未解决异常 → 已配置兜底</span></div><div class="map-body"><div class="loop-map-scroll"><div class="loop-map-canvas" style="width:${width}px;height:${height}px"><svg width="${width}" height="${height}"><defs>${['dependency','planning','recovery'].map(k=>`<marker id="${uid}-${k}" markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto"><path d="M0 0 L7 3.5 L0 7" fill="${k==='recovery'?'#bd705f':k==='planning'?'#9564cf':'#7293bb'}"/></marker>`).join('')}</defs>${paths}</svg>${cards}</div></div><aside class="map-relations"><p data-map-help>点击连线查看推进条件，选中节点查看输入来源和候选实现。循环安排创建新 Task；重试保留同一 Task，在执行记录中查看。</p>${details}</aside></div>${executionOverviewHTML(item,bindings)}${noticeScheduleHTML(item,bindings)}${loopProgressHTML(item,fallback,bindings)}</section>`;
}
...
function setGraphLayer(root,layer){root.dataset.graphLayer=layer;root.classList.toggle('states-view',layer==='states');root.querySelectorAll('[data-graph-layer]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.graphLayer===layer)));}
document.addEventListener('click',e=>{const button=e.target.closest('button[data-graph-layer]');if(button)setGraphLayer(button.closest('.loop-map'),button.dataset.graphLayer);});
