...
function loopProgressHTML(item,fallbackOverride,bindings=item.settings?.bindings||{}){
  ...
  const retries=Object.keys(bp.nodes).some(id=>lifecycleOf(chosenImplementation(item,id,bindings))?.transitions.some(r=>r.to==='retry'));
