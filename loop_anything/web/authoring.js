...
function candidateItem(id='',c={kind:'agent'}){return `<details class="author-candidate author-item" data-last-id="${esc(id)}" data-original="${esc(pretty(c))}" open><summary>${esc(id||'新实现')}</summary><label>实现 ID<input data-candidate-id value="${esc(id)}"></label><label>执行方式<select data-candidate-kind>${['agent','command','external','event','approval'].map(k=>`<option value="${k}" ${c.kind===k?'selected':''}>${esc({agent:'Agent 执行',command:'同步脚本',external:'异步提交＋附属监控',event:'等待外部事件',approval:'等待审批'}[k])}</option>`).join('')}</select></label>${lifecycleEditor(c)}<details class="candidate-parameters"><summary>此实现的任务参数</summary><label class="inline-check"><input type="checkbox" data-candidate-parameters-enabled ${c.parameter_schema?'checked':''}>声明参数要求</label><p class="small muted">安排任务时检查；同时满足节点的参数要求，不自动填值。</p><div data-candidate-parameters ${c.parameter_schema?'':'hidden'}>${schemaForm(c.parameter_schema||{type:'object'})}</div></details><label>工作目录（可选）<input data-candidate-cwd value="${esc(c.cwd||'')}"></label><label>执行 / 提交命令（每行一个参数）<textarea data-candidate-command rows="4">${esc((c.command||[]).join('\n'))}</textarea></label><section data-candidate-monitor ${c.kind==='external'?'':'hidden'}><h4>◉ 附属监控</h4><p class="small muted">接收外部任务标识，查询并报告状态。检查记录归属于当前任务。</p><label>检查命令（每行一个参数）<textarea data-candidate-observe rows="3">${esc((c.observe||[]).join('\n'))}</textarea></label></section><div data-candidate-prompt ${c.kind==='agent'?'':'hidden'}>${promptEditor('task',c)}</div><label data-candidate-event-label ${c.kind==='event'?'':'hidden'}>事件名称<input data-candidate-event value="${esc(c.event||'')}"></label><label>单次调用超时秒数（Agent 留空不限时）<input data-candidate-timeout type="number" min="1" value="${esc(c.timeout||'')}"></label><button type="button" data-remove-author-item>删除实现</button></details>`;}
...
function authorPortButtons(id,n){const steps=Object.entries(editor.loop_definition.plans[authorPlan]?.steps||{}).filter(([,s])=>s.node===id);return `<div class="editor-port-buttons"><div>${Object.keys(n.inputs).map(p=>`<button type="button" data-port-node="${esc(id)}" data-input-port="${esc(p)}" class="${nodeInputRows(editor.loop_definition,id).some(r=>r.port===p&&r.missing)?'input-gap':''}" title="${esc(nodeInputRows(editor.loop_definition,id).filter(r=>r.port===p).map(r=>(r.plan||'入口')+' / '+r.text).join('；')||'创建任务时指定来源')}">◉ ${esc(p)}</button>`).join('')}</div><div>${Object.keys(n.outputs).map(p=>`<button type="button" data-port-node="${esc(id)}" data-output-port="${esc(p)}" class="${portConnection?.node===id&&portConnection?.port===p?'chosen':''}">${esc(p)} ◉</button>`).join('')}</div></div>${steps.length>1?`<select data-node-step="${esc(id)}" aria-label="${esc(id)} 的模板步骤">${steps.map(([k])=>`<option>${esc(k)}</option>`).join('')}</select>`:''}`;}
