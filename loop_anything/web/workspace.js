...
function taskSummary(item){if(item.tasks.status==='skipped')return '历史记录：已跳过，不再等待执行';if(item.tasks.status==='cancelled')return '已取消，不再等待执行';const e=item.attempts.at(-1);if(e?.status==='waiting'&&['timer','event','approval'].includes(e.implementation.kind))return waitSummary(e.implementation);if(e?.observation_error)return '监控需要恢复 · 外部状态待确认';const values=Object.entries(e?.outputs || {});return values.slice(0,2).map(([k,v])=>`${k}：${typeof v==='object'?(Array.isArray(v)?v.length+' 项':'已保存'):String(v).slice(0,64)}`).join(' · ') || (e?.lifecycle_state?'当前阶段：'+lifecycleName(e.lifecycle_state):'');}
function taskCard(item){
  const t=item.tasks,order=taskNumbers.get(t.id);
  return `<button class="process-task ${selectedTask===t.id?'picked':''}" data-status="${esc(t.status)}" data-task-focus="${esc(t.id)}" data-task-row="${esc(t.id)}" aria-pressed="${selectedTask===t.id}" title="${esc(taskLabel(t)+' · '+t.id)}"><span class="task-card-top"><small class="task-number">${String(order).padStart(2,'0')}</small>${badge(t.status)}${taskChanges.has(t.id)?'<span data-change-mark>已调整</span>':''}</span><span class="task-name">${esc(taskLabel(t))}</span><span class="task-summary">${esc(taskSummary(item)||(['fault','stale'].includes(t.status)?'查看失败详情 →':'等待结果'))}</span><small class="task-card-time">${esc([item.round,item.started===null&&['skipped','cancelled'].includes(t.status)?'未执行':taskTime(item.started),item.legacyTime?'历史派发时间':null].filter(Boolean).join(' · '))}</small></button>`;
}
...
function taskError(attempt){
  ...
  const summary=attempt.failure_kind==='command_start'?'命令启动配置错误':attempt.failure_kind==='agent_command'?'Agent 命令异常':attempt.observation_error?'监控异常，外部工作状态未知':raw.includes('Agent exited without sealing')?'Agent 未成功提交结果就退出了。':raw.includes('Agent exited without calling finish')?'Agent 退出前未释放操作权。':'本次执行未能成功完成。';
  const advice=attempt.failure_kind?`请核对命令、目录及 Agent 信任/登录配置；这不是外部业务失败的证明。工作目录：${attempt.implementation.cwd||'平台进程工作目录'}`:attempt.observation_error?'检查已停止，保留原外部任务，不自动重提。':'查看原始错误，核对本次执行和工具回执。';
  const route=run.tasks[attempt.task_id]?.origin.fallback?'<p class="small muted">兜底 Agent 本次未完成；按现有恢复规则处理，连续失败三次后暂停并通知。</p>':'<div class="fallback-summary">'+fallbackSummary(run,run.settings.fallback_node,run.settings.bindings)+'</div>';
  return `<section class="task-error" role="note"><strong>${esc(summary)}</strong><p>${legacy?'这是旧版本运行保留的失败记录。':esc(advice)}</p><details><summary>原始错误与证据路径</summary><pre>${esc(raw)}</pre></details></section>${!legacy&&attempt.status==='fault'?route:''}`;
}
...
function taskWaitReasons(task){
  if(['skipped','cancelled'].includes(task.status)||!task.wait_reasons?.length)return '';
...
function taskDetails(item){
  ...
  ${attempt?.status==='waiting'&&['timer','event'].includes(attempt.implementation.kind)&&!ended?`<button data-task-cancel="${esc(t.id)}">取消等待</button>`:''}${taskFlowHTML(item)}<section class="task-execution"><h3>执行记录 · ${item.attempts.length} 次</h3><p class="small muted">重试记录在同一任务下；每次保留当时的参数、实现和状态变化。</p>${item.attempts.length?`<label>执行尝试<select id="task-attempt">${item.attempts.map(e=>`<option value="${esc(e.id)}" ${e.id===attempt?.id?'selected':''}>第 ${e.attempt} 次 · ${esc(taskRevisionText(e.task_revision))} · ${esc(labels[e.status]||e.status)} · ${taskTime(TaskHistory.startTime(e))}${Object.hasOwn(e,'started_at')?'':' · 历史派发时间'}</option>`).join('')}</select></label>`:''}${promptHistory(attempt)}${taskError(attempt)}<h3>结果</h3>${Object.entries(attempt?.outputs || {}).map(([port,v])=>`<section class="result-section"><h4>${esc(run.loop_definition.records[node.outputs[port]?.record_type]?.title || port)}</h4>${resultWithSchema(v,run.loop_definition.records[node.outputs[port]?.record_type] || {},attempt,port)}</section>`).join('') || (t.status==='skipped'?'<p class="muted">历史任务已跳过，没有提交结果。</p>':'<p class="muted">尚未提交结果</p>')}
  ${waitDetails(attempt)}${executionLifecycle(attempt)}<h3>输入与来源</h3>${Object.entries(attempt?.inputs || {}).map(([port,value])=>`<section><h4>${esc(node.inputs[port]?.title || port)}</h4>${resultView(value)}${sourceLinks(attempt.sources?.[port])}</section>`).join('') || resultView(t.spec.inputs)}
...
function renderTaskHistory(){
  ...
  const groups=taskOrder==='round'?TaskHistory.groups(items):[{round:'已开始',items:items.filter(i=>i.started!==null)},{round:'尚未开始',items:items.filter(i=>i.started===null&&!['completed','skipped','cancelled','stale'].includes(i.tasks.status))},{round:'未执行且已结束',items:items.filter(i=>i.started===null&&['completed','skipped','cancelled','stale'].includes(i.tasks.status))}];
  return `<div class="task-heading"><div><h2>任务关系与推进 ${!run.operator_protocol?'<span class="history-label">历史运行 · 只读</span>':''}</h2><p class="muted small">${all.length} 项任务 · ${all.filter(i=>i.tasks.status==='completed').length} 已完成 · ${all.filter(i=>['fault','stale'].includes(i.tasks.status)).length} 失败 · ${all.filter(i=>i.started===null&&!['completed','skipped','cancelled','stale'].includes(i.tasks.status)).length} 尚未开始${all.some(i=>i.tasks.status==='skipped')?' · '+all.filter(i=>i.tasks.status==='skipped').length+' 已跳过':''}</p></div><div class="toolbar"><button id="add-run-task" ${!run.operator_protocol||['completed','terminated'].includes(run.status)?'disabled':''}>＋ 添加任务</button><button id="add-run-batch" ${run.operator_protocol&&Object.keys(run.loop_definition.plans||{}).length&&!['completed','terminated'].includes(run.status)?'':'disabled'}>＋ 按模板安排</button></div></div><div class="process-controls"><div class="segmented">${[['graph','任务关系'],['time','时间顺序'],['round','按轮次']].map(([id,label])=>`<button data-task-order="${id}" class="${taskOrder===id?'selected':''}">${label}</button>`).join('')}</div><label class="task-search-label"><span class="sr-only">查找任务</span><input id="task-search" placeholder="查找任务…" type="search" value="${esc(taskSearch)}"></label><label class="task-status-label"><span class="sr-only">任务状态</span><select id="task-status"><option value="">全部状态</option>${[...new Set(all.map(i=>i.tasks.status))].map(s=>`<option value="${esc(s)}" ${taskStatus===s?'selected':''}>${esc(labels[s]||s)}</option>`).join('')}</select></label>${filterNode?'<button id="clear-task-filter">清除节点筛选</button>':''}</div>
  <div class="process-layout"><section class="process-body">${items.length?(taskOrder==='graph'?taskDependencyGraph(items):groups.map(g=>`<details class="task-group" open><summary>${esc(g.round || '未标注轮次')} · ${g.items.length}</summary><div class="process-list">${g.items.map(taskCard).join('')}</div></details>`).join('')):'<p class="empty-inline">没有符合条件的任务</p>'}</section><aside id="task-detail" class="task-detail">${taskDetails(all.find(i=>i.tasks.id===selectedTask))}</aside></div>${run.history.filter(h=>h.kind==='plan_built'&&h.detail?.omitted?.length).map(h=>`<details class="omitted-steps"><summary>未安排的步骤 · ${esc(h.detail.name)} · ${esc(taskTime(h.at))}</summary>${omittedStepsHTML(h.detail.omitted,h.detail.name)}</details>`).join('')}${loopProgressHTML(run)}`;
}
function recordChoices(){const m=new Map();for(const [id,versions] of Object.entries(run.records)){const r=versions.at(-1);m.set(id,{id,label:id+' · v'+r.revision});}for(const t of Object.values(run.tasks))for(const [port,d] of Object.entries(t.spec.outputs)){if(!m.has(d.id)&&!['cancelled','stale','skipped'].includes(t.status))m.set(d.id,{id:d.id,label:taskLabel(t)+' / '+port+' · 尚未产生'});}return [...m.values()];}
...
function omittedStepsHTML(items,planName){return items.length?`<section class="omitted-summary"><h3>未安排 · 不创建任务</h3>${items.map(item=>`<p><strong>${esc(run.loop_definition.nodes[run.loop_definition.plans[item.plan||planName]?.steps[item.step]?.node]?.label||item.step)}</strong> · ${esc(({not_selected:'本次未选择',condition_false:'安排条件不满足',empty_items:'展开列表为空'})[item.reason]||item.reason)}</p>${item.when?`<details><summary>安排条件</summary><pre>${esc(pretty(item.when))}</pre></details>`:''}`).join('')}</section>`:'';}
function renderBatchFields(){const p=run.loop_definition.plans[$('batch-plan').value];$('batch-fields').innerHTML=`<div id="batch-values">${valueField({},p.parameters || {type:'object'},'本批参数')}</div>${Object.entries(p.steps).map(([key,s])=>`<details data-batch-step="${esc(key)}"><summary>${esc(run.loop_definition.nodes[s.node].label||s.node)} · ${esc(key)}</summary><label class="inline-check"><input data-batch-include type="checkbox" checked>安排此步骤${s.when?'（满足模板条件时）':''}</label>${s.when?`<details><summary>查看安排条件</summary><pre>${esc(pretty(s.when))}</pre></details>`:''}<label class="inline-check"><input data-batch-override type="checkbox">改用已有输入（例如省略了上游步骤）</label><div data-batch-sources hidden>${Object.entries(run.loop_definition.nodes[s.node].inputs).map(([port,schema])=>sourceField(port,{},schema,recordChoices())).join('')}</div></details>`).join('')}`;}
