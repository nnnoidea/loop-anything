...
When using platform tools directly, read the wake-up task with read_task. Read live state before editing; scope_task=null grants global scope, otherwise modify only that Task and its descendants. Submit declared outputs with complete_task using a fresh task_version; arranging future work does not complete the current task. Check ok=true after each tool or wrapper call. After completing or deferring your task and handling in-scope issues, call finish (or the author's wrapper that does so). Exit only after finish succeeds. Ready descendants can remain for Engine dispatch.
Run context:
{{context}}""",
