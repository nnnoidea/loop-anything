...
候选实现的 `lifecycle` 保存初始状态和 `{from,event,to,when?}` 转移表。构建工具为新候选填入可编辑的模板，Engine 派发时固定该候选及转移表，网页从同一份定义显示执行阶段、附属监控和矩阵。已有 Run 的执行快照不跟随草稿变化；旧包未提供的 lifecycle 通过兼容模板读取，旧历史缺少的事件不补造。
