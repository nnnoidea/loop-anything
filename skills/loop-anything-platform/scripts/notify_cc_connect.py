...
def main():
    ...
    if args.print_command:
        ...
        return
    if not all(v and v.strip() for v in (args.project, args.session, args.data_dir)):
        parser.error('The destination must be explicit and nonempty')
    try:
        ...
        result = subprocess.run([args.executable, 'send', '--project', args.project, '--session', args.session,
                                 '--data-dir', args.data_dir, '--stdin'], input='\n'.join(lines), text=True,
